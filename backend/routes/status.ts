      ...
      try {
        ...
        pool.connect(async (sqlError: Error, client: PoolClient) => {
          ...
          const getMinValue = async (table: string, column: string) => {
            const result = await client.query("SELECT get_min_value($1, $2)", [
              table,
              column,
            ])
          ...
          const getMaxValue = async (table: string, column: string) => {
            const result = await client.query("SELECT get_max_value($1, $2)", [
              table,
              column,
            ])
          ...
          const inputs = await Promise.all(
            Config.inputSets
              .reduce(
                (a: any, c: any) => [...a, ...c.inputs],
                []
              ) /* Concatenate the sliders from all slider sets into a single array */
              .map(async (s: any) => {
                ...
                return {
                  ...
                  min:
                    s.table !== "" && s.column !== ""
                      ? await getMinValue(s.table, s.column)
                      : s.min,
                  max:
                    s.table !== "" && s.column !== ""
                      ? await getMaxValue(s.table, s.column)
                      : s.max,
                }
              })
